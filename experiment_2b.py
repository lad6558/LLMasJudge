...
async def run_experiment(num_trials: int = 10, scale: int = 10, cutoff: int = 80):
    """
    Run experiment 2b: Testing if post-hoc reasoning affects judgment variance
    
    This experiment:
    1. Uses GPT-4o-mini to score GPT-4 responses
    2. Tests temperatures [0.3, 0.5, 1.0]
    3. Requires step-by-step reasoning after scoring
    4. Tests hypothesis that post-hoc reasoning won't reduce variance
    ...
    # Create exp2b subdirectory
    exp_dir = results_dir / "exp2b"
    ...
    all_results = {
        "experiment": "2b",
        "description": "Testing if post-hoc reasoning affects judgment variance",
        "parameters": {
            "scale": scale,
    ...
    for temp in temp_pbar:
        ...
        try:
            print(f"\nStarting evaluation with post-hoc reasoning")
            variances = await evaluate_all_responses(
                ...
                temperature=temp,
                cutoff=cutoff,
                reasoning="after"  # Enable post-hoc reasoning
            )
...
def main():
    parser = argparse.ArgumentParser(
        description='Run experiment 2b: Testing if post-hoc reasoning affects judgment variance')
