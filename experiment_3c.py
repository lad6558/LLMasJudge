...
from core_3c import evaluate_all_responses
import asyncio
from tqdm import tqdm

async def run_experiment(num_trials: int = 10, scale: int = 10, cutoff: int = 80):
    """
    Run experiment 3c: Testing model size vs judgment variance with detailed scoring scale
    
    This experiment:
    1. Uses GPT-3.5-turbo, GPT-4o-mini, and GPT-4o to score GPT-4 responses
    2. Uses detailed 1-10 scoring scale with specific descriptions
    3. Tests temperatures from 0.0 to 1.0
    4. Computes variance in scores for each model
    """
    ...
    # Create exp3c subdirectory
    exp_dir = results_dir / "exp3c"
    ...
    all_results = {
        "experiment": "3c",
        "description": "Testing model size vs judgment variance with detailed scoring scale",
        "parameters": {
            "scale": scale,
...
def main():
    parser = argparse.ArgumentParser(description='Run experiment 3c: Testing model size vs judgment variance with detailed scoring scale')
